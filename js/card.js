...
const formAds = ({title, address, price, type, rooms, guests, checkin, checkout, features, description, photos}, {avatar}) => {
  const wizardElement = templateFragment.cloneNode(true);
  wizardElement.querySelector('.popup__title').textContent = title;
  wizardElement.querySelector('.popup__text--address').textContent = address.location.x + ' | ' + address.location.y;
  ...
  wizardElement.querySelector('.popup__photo').src = photos;
